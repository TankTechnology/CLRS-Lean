...
lemma rpow_sub_floor_le_of_two_le {p : ℝ} (hp : 0 < p) {x : ℝ} (hx2 : 2 ≤ x) :
  ...
  · have hfloor_le : (⌊x⌋₊ : ℝ) ≤ x := Nat.floor_le hx0.le
    ...
    have hmvt : ∃ c ∈ Set.Ioo (⌊x⌋₊ : ℝ) x,
      ...
      · exact (continuous_rpow_const hp.le).continuousOn
      · intro z hz
        rw [Set.mem_Ioo] at hz
        have hz_pos : 0 < z := lt_of_lt_of_le (by norm_num : (0 : ℝ) < 1)
          (le_trans hfloor_ge_one (le_of_lt hz.1))
        exact Real.differentiableAt_rpow_const_of_ne p (ne_of_gt hz_pos)
    ...
    have hcp : c ^ (p - 1) ≤ 2 * x ^ (p - 1) := by
      ...
      · have hp_lt_one : p < 1 := lt_of_not_ge hp1
        ...
        have hhalf : (x / 2) ^ (p - 1) = 2 ^ (1 - p) * x ^ (p - 1) := by
          rw [Real.div_rpow hx0.le (by norm_num : 0 ≤ (2 : ℝ))]
          rw [Real.rpow_neg (by norm_num : 0 ≤ (2 : ℝ)) (1 - p)]
          congr 1
          ring
        calc c ^ (p - 1) ≤ (x / 2) ^ (p - 1) := Real.rpow_le_rpow_of_nonpos hc_pos hc_ge_half hneg
          _ = 2 ^ (1 - p) * x ^ (p - 1) := hhalf
          _ ≤ 2 * x ^ (p - 1) := by
            have h2 : 2 ^ (1 - p) ≤ 2 := by
              exact Real.rpow_le_rpow_of_exponent_le (by norm_num : 1 ≤ (2 : ℝ)) (by linarith : 1 - p ≤ 1)
