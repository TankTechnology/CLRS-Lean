...
* **Theorem 22.9**: the system is feasible `↔` the constraint graph has no
...
- `CLRS.Chapter24.WeightedGraph.DiffConstraintSystem.noNegCycle_of_feasible`:
  feasible assignment `⇒` no negative-weight cycle (Theorem 22.9, forward).
- `CLRS.Chapter24.WeightedGraph.DiffConstraintSystem.feasible_of_noNegCycle`:
  ...
  (Theorem 22.9, reverse).
- `CLRS.Chapter24.WeightedGraph.DiffConstraintSystem.diffConstraint_feasible_iff_noNegCycle`:
  **CLRS Theorem 22.9** — the full equivalence, with explicit Bellman-Ford solution.
...
then the constraint graph has no negative-weight cycle (Theorem 22.9, forward). -/
