...
Appending a key to a full-prefix max-heap can invalidate only the new cell's
incoming edge.  The existing upward-exception invariant and bubble loop provide
the complete repair proof.
-/
