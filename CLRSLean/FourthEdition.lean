import CLRSLean.FourthEdition.Chapter_01
import CLRSLean.FourthEdition.Chapter_02
import CLRSLean.FourthEdition.Chapter_03
import CLRSLean.FourthEdition.Chapter_04
import CLRSLean.FourthEdition.Chapter_05
import CLRSLean.FourthEdition.Chapter_06
import CLRSLean.FourthEdition.Chapter_07
import CLRSLean.FourthEdition.Chapter_08
import CLRSLean.FourthEdition.Chapter_09
import CLRSLean.FourthEdition.Chapter_10
import CLRSLean.FourthEdition.Chapter_11
import CLRSLean.FourthEdition.Chapter_12
import CLRSLean.FourthEdition.Chapter_13
import CLRSLean.FourthEdition.Chapter_14
import CLRSLean.FourthEdition.Chapter_15
import CLRSLean.FourthEdition.Chapter_16
import CLRSLean.FourthEdition.Chapter_17
import CLRSLean.FourthEdition.Chapter_18
import CLRSLean.FourthEdition.Chapter_19
import CLRSLean.FourthEdition.Chapter_20
import CLRSLean.FourthEdition.Chapter_21
import CLRSLean.FourthEdition.Chapter_22
import CLRSLean.FourthEdition.Chapter_23
import CLRSLean.FourthEdition.Chapter_24
import CLRSLean.FourthEdition.Chapter_25
import CLRSLean.FourthEdition.Chapter_26
import CLRSLean.FourthEdition.Chapter_27
import CLRSLean.FourthEdition.Chapter_28
import CLRSLean.FourthEdition.Chapter_29
import CLRSLean.FourthEdition.Chapter_30
import CLRSLean.FourthEdition.Chapter_31
import CLRSLean.FourthEdition.Chapter_32
import CLRSLean.FourthEdition.Chapter_33
import CLRSLean.FourthEdition.Chapter_34
import CLRSLean.FourthEdition.Chapter_35

/-!
# CLRS fourth edition

This is the canonical chapter tree for CLRS-Lean.  During the compatibility
period the guides reuse theorem-bearing modules whose paths follow the third
edition.  New work should start from a {lit}`CLRSLean.FourthEdition.Chapter_NN`
guide and consult {lit}`docs/clrs-fourth-edition-map.csv` before choosing a
section module.

The {lit}`FourthEdition` prefix is transitional.  At the cleanup major release,
unqualified {lit}`CLRSLean.Chapter_NN` paths will adopt fourth-edition meanings.
-/
