...
This file formalizes the bipartite-to-flow-network reduction of CLRS §26.3
and proves Theorem 26.12: the maximum matching size equals the maximum flow
...
- `Flow.IsIntegral`, `matchingOfIntegralFlow`, and
  ...
  matching of size `v`
- `maxMatching_eq_maxFlow_value` (Theorem 26.12): the maximum matching size
...
on pair `(u,v)` (CLRS eq. (26.11)): `+1` along `s → e.1`, `e.1 → e.2`,
...
/-- The feasible flow induced by a matching (CLRS §26.3).  The flow sends one
...
the matching network yields a matching of size `v` (CLRS Theorem 26.12,
converse direction). -/
...
/-! ## Integral maximum flow and Theorem 26.12 -/
...
/-- **Theorem 26.12 (CLRS).**  In the unit-capacity network of a bipartite
