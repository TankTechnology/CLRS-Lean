...
{lit}`not-started`, and the generated dashboard reports all 1,632 selected
...
Chapter 2 now includes a local executable MERGE whose bundled contract proves
sortedness, exact permutation preservation, a linear head-comparison bound,
and exactly one output write per input element.  The remaining line-by-line
pseudocode and word-RAM accounting are deliberately tracked as lower-level
refinements rather than implied by that theorem.
...
factorization.  Those 44 groups are disjoint from the 1,632 canonical tracked
theorem entries.
