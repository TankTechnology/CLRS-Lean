...
- {lit}`CLRS.Chapter24.WeightedGraph.lemma_25_1`: Lemma 23.1 (`L^(m+1) = L^m ◁ W`).
- {lit}`CLRS.Chapter24.WeightedGraph.L_sq_eq_minPlusMul`: Lemma 23.2 (`L^(2m) = L^m ◁ L^m`).
...
/-! ## Lemmas 23.1 and 23.2 (squaring identity) -/
