...
def check_modified():
    """At B2 windows: what does e evict at modified fault positions in (t, J]?"""
    alpha = [1, 2, 3, 4]
    cases = []
    for n in range(4, 10):
        for sig_t in product(alpha, repeat=n):
                    ...
                    while True:
                        ...
                        if t >= hnb:
                            ...
                            if isB2:
                                ...
                                if J is not None:
                                    for pos in range(t + 1, J + 1):
                                        if pos in modified and sig[pos] not in s.sched_cache(e, sig, C0, pos):
                                            cases.append((sig, tuple(sorted(C0)), name, t, q, qp, j, pos,
                                                          sig[pos], e(pos), d(pos)))
                            if isB2:
                                j = s.next_use(sig, t + 1, d(t))
                                if j is not None:
                            ...
                            if jp is not None:
                                hnb = max(hnb, t + 1 + jp + 1)
    print(f"modified-fault cases: {len(cases)}")
    for c in cases[:12]:
        sig, C0, name, t, q, qp, j, pos, sigp, ep, dp = c
        print(f"  sig={sig} C0={C0} d0={name} t={t} q={q} q''={qp} j={j} "
              f"pos={pos} sig[pos]={sigp} e[pos]={ep} d[pos]={dp} "
              f"e[pos]==q: {ep==q}  e[pos]==q'': {ep==qp}  sig[pos]==q: {sigp==q}  sig[pos]==q'': {sigp==qp}")
