      ...
      · subst s
          ...
          · -- e 命中,r 缺页(nop 逐出 no-op,Ŝ 载入 q'')
            ...
            · have hmem : x ∈ E (t + 1 + j'') \ S (t + 1 + j'') := by
                ...
                exact ⟨hx.1, by
                  ...
                  exact hx.2 (by
                    rw [hsig]
                    exact Finset.mem_insert.mpr (Or.inr hxS))⟩
        ...
        · -- e 缺页
          ...
          · -- e 缺页,r 命中
            ...
            · have hmem : x ∈ E (t + 1 + j'') \ S (t + 1 + j'') := by
                rw [Finset.mem_sdiff]
                exact ⟨(Finset.mem_erase.mp hx.1).2, by
                  intro hxS
                  exact hx.2 (by
                    exact Finset.mem_insert.mpr (Or.inr hxS))⟩
              have hxq := Finset.mem_singleton.mp (hw (t + 1 + j'') (by omega) le_rfl hmem)
          ...
          · -- 双缺页
            ...
            · have hmem : x ∈ E (t + 1 + j'') \ S (t + 1 + j'') := by
                ...
                exact ⟨(Finset.mem_erase.mp hx.1).2, by
                  intro hxS
                  exact hx.2 (Finset.mem_insert.mpr (Or.inr hxS))⟩
              have hxq := Finset.mem_singleton.mp (hw (t + 1 + j'') (by omega) le_rfl hmem)
              exact hx.2 (by
                rw [← hxq, ← hsig]
                exact Finset.mem_insert_self _ _)
        ...
        · -- e 命中 ⟹ r 命中(E_s ⊆ Ŝ_s)
          ...
          have hr' : σ.getD s 0 ∈ S s := by
            ...
            have hmem : σ.getD s 0 ∈ E s \ S s := by
              rw [Finset.mem_sdiff]
              exact ⟨he, hnot⟩
            exact (Finset.not_mem_empty (σ.getD s 0)) (hih ▸ hmem)
          rw [if_pos hr'] at hx
          exact (Finset.not_mem_empty x) (hih ▸ (by
            rw [Finset.mem_sdiff]
            exact ⟨hx.1, hx.2⟩))
        · -- e 缺页
          rw [if_neg he] at hx
          ...
          · -- r 命中:Ŝ 不变,E 载入
            ...
            exact (Finset.not_mem_empty x) (hih ▸ (by
              ...
              exact ⟨(Finset.mem_erase.mp hx.1).2, by
                intro hxS
                by_cases hxeq : x = σ.getD s 0
                · exact he (hxeq ▸ (Finset.mem_erase.mp hx.1).2)
                · exact hx.2 (Finset.mem_insert.mpr (Or.inr hxS))⟩))
          · -- 双缺页
            rw [if_neg hr'] at hx
            exact (Finset.not_mem_empty x) (hih ▸ (by
  ...
  · -- J < s
    ...
    · intro x hx
      ...
      · -- J'' < s:∅
        ...
        exfalso
        exact (Finset.not_mem_empty x) (hempty ▸ hx)
    · intro hs1
      by_cases hsJ'' : s ≤ t + 1 + j''
      · exact hw s hsJ' hsJ''
      · exfalso
