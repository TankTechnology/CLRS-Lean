...
{lit}`δ` of Section 22.1, this section formalizes the correctness core of
...
- {lit}`CLRS.Chapter24.WeightedGraph.noNegCycle_of_nonneg`: nonnegative weights
  have no negative cycle, so Section 22.1's {lit}`δ` machinery applies.
...
- {lit}`CLRS.Chapter24.WeightedGraph.dijkstra_extractMin_correct`: **CLRS
  Theorem 22.6 (greedy invariant)** — under the Dijkstra relaxation invariants,
...
- {lit}`δ` : true shortest-path distances (from Section 22.1)
...
/-- **CLRS Theorem 22.6 (Dijkstra greedy invariant).**  Suppose the source {lit}`s` is
