...
This section formalizes the maximum-flow problem model from CLRS Chapter 24.  We
...
the flow value (Lemma 24.5) and the generic Ford-Fulkerson correctness
...
- `Flow.netFlow_eq_value` (Lemma 24.5): for any cut `(S,T)` with
...
`Section_24_6_MaxFlow_MinCut`.  Edmonds-Karp analysis and the executable
...
/-- **Lemma 24.5 (CLRS).**  For any cut `(S, T)` with source in `S` and sink
...
of the Max-Flow Min-Cut Theorem, CLRS Theorem 24.6).  It reuses the saturated
