...
Trivial cost bounds for {lit}`MAX-HEAPIFY` ({lit}`O(n)`), {lit}`BUILD-MAX-HEAP`
({lit}`O(n²)`), and {lit}`HEAPSORT` ({lit}`O(n²)`) using {lit}`Nat.log 2 n ≤ n`.
Tight bounds ({lit}`O(log n)`, {lit}`O(n log n)`) deferred.
-/

open Chapter03

/-- Worst-case cost of one MAX-HEAPIFY call. -/
noncomputable def maxHeapifyCost (n : ℕ) : ℕ := Nat.log 2 n + 1

lemma maxHeapifyCost_le (n : ℕ) (hn : 1 ≤ n) : (maxHeapifyCost n : ℝ) ≤ 2 * (n : ℝ) := by
  unfold maxHeapifyCost
  have h_log : (Nat.log 2 n : ℝ) ≤ (n : ℝ) := by exact_mod_cast Nat.log_le_self 2 n
  have hn' : (1 : ℝ) ≤ (n : ℝ) := by exact_mod_cast hn
  push_cast
  linarith

theorem maxHeapifyCost_isBigO_n :
    isBigO (fun n : ℕ => (maxHeapifyCost n : ℝ)) (fun n : ℕ => (n : ℝ)) := by
  rw [isBigO_iff]
  refine ⟨2, by norm_num, 1, fun n hn => ?_⟩
  have h_nonneg : 0 ≤ (maxHeapifyCost n : ℝ) := by positivity
  rw [abs_of_nonneg h_nonneg, abs_of_nonneg (Nat.cast_nonneg _)]
  exact maxHeapifyCost_le n hn

noncomputable def buildMaxHeapCost (n : ℕ) : ℕ := (n / 2) * maxHeapifyCost n

lemma buildMaxHeapCost_le (n : ℕ) (hn : 1 ≤ n) : (buildMaxHeapCost n : ℝ) ≤ 2 * ((n : ℝ) * (n : ℝ)) := by
  unfold buildMaxHeapCost; push_cast
  have h_div : ((n / 2 : ℕ) : ℝ) ≤ (n : ℝ) := by
    have : (n / 2 : ℕ) ≤ n := Nat.div_le_self n 2
    exact_mod_cast this
  have h_cost := maxHeapifyCost_le n hn
  nlinarith

theorem buildMaxHeapCost_isBigO_nsq :
    isBigO (fun n : ℕ => (buildMaxHeapCost n : ℝ)) (fun n : ℕ => (n : ℝ) * (n : ℝ)) := by
  rw [isBigO_iff]
  refine ⟨2, by norm_num, 1, fun n hn => ?_⟩
  have h_nonneg : 0 ≤ (buildMaxHeapCost n : ℝ) := by positivity
  rw [abs_of_nonneg h_nonneg, abs_of_nonneg (by positivity : 0 ≤ (n : ℝ) * (n : ℝ))]
  exact buildMaxHeapCost_le n hn

noncomputable def heapSortCost (xs : List Nat) : ℕ :=
  buildMaxHeapCost xs.length + xs.length * maxHeapifyCost xs.length

lemma heapSortCost_le (n : ℕ) (hn : 1 ≤ n) : (heapSortCost (List.replicate n 0) : ℝ) ≤ 4 * ((n : ℝ) * (n : ℝ)) := by
  unfold heapSortCost
  simp [List.length_replicate]
  have h_build := buildMaxHeapCost_le n hn
  have h_cost := maxHeapifyCost_le n hn
  push_cast; nlinarith

theorem heapSortCost_isBigO_nsq :
    isBigO (fun n : ℕ => (heapSortCost (List.replicate n 0) : ℝ))
      (fun n : ℕ => (n : ℝ) * (n : ℝ)) := by
  rw [isBigO_iff]
  refine ⟨4, by norm_num, 1, fun n hn => ?_⟩
  have h_nonneg : 0 ≤ (heapSortCost (List.replicate n 0) : ℝ) := by positivity
  rw [abs_of_nonneg h_nonneg, abs_of_nonneg (by positivity : 0 ≤ (n : ℝ) * (n : ℝ))]
  exact heapSortCost_le n hn
