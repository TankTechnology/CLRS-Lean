...
/-!
# HAM-CYCLE incidence scanner: left endpoint comparison
-/
...
/-- Exact controller cost for comparing a left endpoint. -/
...
private def leftField_run_aux (remaining saved occurrence candidate : Nat)
    ...
    EvalsToInTime (step program)
      ...
      (some (cfg (.right (equal && decide (remaining = candidate)) true)
  ...
  | zero =>
      let afterPop := cfg (.leftEnd equal) buffer₁ (some (some .pairSep))
        test (rest.map some) output work₁ (some CliqueSym.pairSep :: work₂)
      ...
      have first : EvalsToInTime (step program)
        ...
        ⟨⟨1, by simp [flip, afterPop, prependCliqueTicks, step, program,
          cfg, stepOp]⟩, le_rfl⟩
      have finish := leftEnd_run remaining saved occurrence equal (rest.map some)
        output work₁ (some CliqueSym.pairSep :: work₂) buffer₁
        (some (some .pairSep)) test
      have restore := restoreLeft_run (remaining + saved) 0 occurrence
      ...
      simpa [leftFieldSteps, prependCliqueTicks, Nat.add_assoc, Nat.add_comm,
        Nat.add_left_comm] using full
  | succ candidate ih =>
      let remainingInput :=
        (prependCliqueTicks candidate (.pairSep :: rest)).map some
      let afterPop := cfg (.spendLeft equal) buffer₁ (some (some .tick)) test
        remainingInput output work₁ (some CliqueSym.tick :: work₂)
        (List.replicate remaining ()) (List.replicate occurrence ())
        (List.replicate saved ())
      have first : EvalsToInTime (step program)
          (cfg (.left equal) buffer₁ buffer₂ test
            ((prependCliqueTicks (candidate + 1) (.pairSep :: rest)).map some)
      ...
      | zero =>
          let afterSpend := cfg (.left false) buffer₁ (some (some .tick)) false
            remainingInput output work₁ (some CliqueSym.tick :: work₂) []
            (List.replicate occurrence ()) (List.replicate saved ())
          ...
          let full := EvalsToInTime.trans (step program)
            2 (leftFieldSteps 0 saved candidate) _ afterSpend _ throughSpend
            (by simpa [remainingInput] using restRun)
          simpa [leftFieldSteps, prependCliqueTicks, List.reverse_cons,
            List.append_assoc, Nat.add_assoc, Nat.add_comm,
            Nat.add_left_comm] using full
      | succ remaining =>
          let afterSpend := cfg (.saveLeft equal) buffer₁ (some (some .tick))
            true remainingInput output work₁ (some CliqueSym.tick :: work₂)
            (List.replicate remaining ()) (List.replicate occurrence ())
            (List.replicate saved ())
          let afterSave := cfg (.left equal) buffer₁ (some (some .tick)) true
            remainingInput output work₁ (some CliqueSym.tick :: work₂)
            (List.replicate remaining ()) (List.replicate occurrence ())
...
/-- Compare a canonical left endpoint and restore the query counter. -/
def leftField_run (query occurrence candidate : Nat) (equal : Bool)
    (rest : List CliqueSym) (output : List UnaryFrameSym)
    ...
    EvalsToInTime (step program)
      (cfg (.left equal) buffer₁ buffer₂ test
        ...
        (List.replicate occurrence ()) [])
      (some (cfg (.right (equal && decide (candidate = query)) true)
        buffer₁ (some (some .pairSep)) false (rest.map some) output work₁
        (((prependCliqueTicks candidate [.pairSep]).map some).reverse ++ work₂)
        (List.replicate query ()) (List.replicate occurrence ()) []))
      (leftFieldSteps query 0 candidate) := by
  have run := leftField_run_aux query 0 occurrence candidate equal rest output
