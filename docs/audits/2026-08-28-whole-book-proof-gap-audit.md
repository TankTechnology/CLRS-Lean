...
- Chapter 28 target #340 is closed on the same branch: explicit pivot scanning,
  ...
  cubic and includes factor-assembly divisions, while direct permutation
  indexing leaves the costed LUP-SOLVE counter quadratic without hidden
...
| 25 | Berge augmentation, a BFS-selected unit-capacity flow run, Gale--Shapley, and the terminating Hungarian loop are executable at finite mathematical level. | Matching/flow refinement, integrality at every augmentation, stability/perfectness, alternating-tree, potential, and augmentation invariants are proved. | Maximum matching, proposer optimality, and assignment optimality are proved. | The §25.1 run has at most `|V|` augmentations. Its current residual-neighbor operation enumerates all vertices, so the textbook adjacency-list O(VE) bound is not attached; Gale--Shapley and Hungarian termination are bounded. | Finite preference/assignment models are explicit; the adjacency-list flow budget is only a target specification. | `Tests/Trust/Chapter_25.lean`. | `semantic-bridge-gap` (medium): instrument an adjacency-list residual BFS and path update, prove erasure to the flow run, and derive O(VE); issue #339 remains open. |
...
| P2 | 25 | `semantic-bridge-gap` | Maximum-matching correctness and the BFS-selected augmentation bound are proved, but the textbook O(VE) unit-capacity flow execution cost is absent. | Instrumented adjacency-list residual BFS and path update, erasure to the proved flow run, and an attached O(VE) theorem. |
...
- [#339](https://github.com/TankTechnology/CLRS-Lean/issues/339) — Chapter 25 flow-method O(VE) matching cost (open: the BFS correctness/augmentation chain is proved; adjacency-list cost attachment remains).
