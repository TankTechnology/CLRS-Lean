...
| 25 | Berge augmentation, the legacy semantic flow run, a support-indexed costed matching run, Gale--Shapley, and the terminating Hungarian loop are executable at finite mathematical level. | Matching/flow refinement, matching-flow integrality, residual-support coverage, exact BFS-state erasure, parent-path recovery, executable residual-to-graph projection, concrete matching updates, stability/perfectness, alternating-tree, potential, and augmentation invariants are proved. | Maximum matching, maximal integral attached flow, proposer optimality, and assignment optimality are proved. | `costedMatchingRun` builds its support index once, reuses it across attempts, and accumulates bucket-scan, parent-list, graph-projection, and edge-update work; `costedMatchingRun_work_le_product` proves `work ≤ 20·V_f·(E_f+1)`. | The textbook unit-cost RAM support-bucket model is explicit and is not presented as Lean evaluator cost; proof witnesses carry no runtime charge. | `Tests/Chapter_25/CostedBipartiteFlow.lean`; `Tests/Trust/Chapter_25.lean`. | `proved`; the old all-vertices BFS remains a semantic reference and is not used to justify the adjacency-list cost. |
...
The audit confirms six remaining proof targets.  The first three affect the strongest
