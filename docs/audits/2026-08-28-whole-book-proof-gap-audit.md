...
| 25 | Berge augmentation, a BFS-selected costed unit-capacity flow run, Gale--Shapley, and the terminating Hungarian loop are executable at finite mathematical level. | Matching/flow refinement, integrality at every augmentation, stability/perfectness, alternating-tree, potential, and augmentation invariants are proved. | Maximum matching, proposer optimality, and assignment optimality are proved. | The §25.1 run has at most `|V|` augmentations, linear adjacency-list work per attempt, and an attached O(VE) total bound; Gale--Shapley and Hungarian termination are also bounded. | Finite preference/assignment models and the adjacency-list flow cost model are explicit. | `Tests/Trust/Chapter_25.lean`. | `proved`; issue #339 closed by the costed execution/refinement chain. |
...
- [#339](https://github.com/TankTechnology/CLRS-Lean/issues/339) — Chapter 25 flow-method O(VE) matching cost (closed by the costed execution/refinement chain).
