...
- Status: `partial` with the all-input transfer bridge proved
- Main proved theorems:
...
- Proof pattern: assume absolute-value monotonicity for the cost function and
  explicit upper/lower power-sandwich hypotheses for the comparison function;
  use the exact-power O/Ω/Θ bound at a sufficiently large power and transfer it
  back to an arbitrary large input by monotonicity.
- Current gap: prove the concrete power-sandwich hypotheses generated by
  floor/ceiling recurrence models and regularity assumptions, then package the
  final CLRS Master-theorem cases.
...
| Chapter 4 concrete all-input Master-theorem instantiation | `future-work` | Generic all-input transfer is proved; concrete floor/ceiling recurrence sandwich hypotheses remain. |
