...
- [ ] Prove a linear length bound for each normalized-pair row.
- [ ] Prove a quadratic length bound for the complete normalized-pair list and
  its filtered complement-edge list.

## Task 2: Complement-encoding bound

- [ ] Bound every encoded complement edge by a linear function of the vertex
  count.
- [ ] Prove a cubic bound for the encoded complemented instance.

## Task 3: Total raw-map bounds

- [ ] Use successful-decoder field bounds to express the valid-input result in
  terms of the original raw input length.
- [ ] Cover malformed and ill-formed fallbacks and publish bounds for both
  `cliqueToVertexCoverMap` and `vertexCoverToCliqueMap`.

## Task 4: Verification and checkpoint

- [ ] Add a focused interface test for the new bounds.
- [ ] Run the focused test, Chapter 34 root build, axiom audit, placeholder
