...
| Chapter 22, Sections 22.1-22.5 | A finite directed-graph model and reachability-complete BFS are established; the DFS development proves color/timestamp invariants, the white-path characterization, the timestamp parenthesis theorem, the parent-forest ancestor/interval characterization, unique tree/back/forward/cross edge classification, discovery-state bridges, and SCC finish-time ordering; Kahn and CLRS DFS finish-time topological sorts are proved correct for DAGs; Kosaraju's returned blocks are proved to be strongly connected, maximal, pairwise disjoint, covering, and therefore an SCC partition. | BFS shortest-path distance and predecessor-tree lemmas; explicit cost refinements. |
...
| --- | --- | --- |
| 0 | Chapter 22 | Complete BFS unweighted shortest-path distances and predecessor-tree correctness.  The DFS white-path, parenthesis, ancestor/interval, edge-classification, finish-time topological-sort, and full Kosaraju SCC theorem chains are proved. |
