...
| --- | --- | --- | --- |
| P0 | Chapter 34, §34.5 | The §34.4 chain reaches the NP-complete honest graph-plus-`k` `CLIQUE`. VERTEX-COVER has bidirectional typed/raw complement semantics, total membership-preserving raw maps in both directions, and cubic output-length bounds. Both directions factor through one guarded complement-map target parameterized only by the fixed fallback. The first concrete phase is closed: a fixed linear-time TM2 preserves valid graph syntax and maps parser failures to an ill-formed sentinel. `mem_generalVERTEXCOVER_iff_exists_certificate` also gives exact Boolean-certificate semantics with a quadratic bound. The remaining complement machine, NP membership, and NP-completeness are not yet closed. | Add the shared well-formedness guard and runtime pair/nonedge emitter on top of the syntax normalizer, reuse the CLIQUE verifier where possible, then package VERTEX-COVER NP-completeness before HAM-CYCLE → TSP and SUBSET-SUM. |
