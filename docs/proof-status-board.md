...
| Chapter 9, Sections 9.2-9.3 | Selection-by-rank correctness is proved for the specification selector, a pivot-style quickselect model, and a pivot-parametric deterministic SELECT model, using a count-based order-statistic certificate; the local five-element median certificate, executable grouping, and full-input split-count core are also proved. | Randomized SELECT expected time, final `7n/10` partition-size packaging, and runtime analysis. |
...
| Chapter 9 linear-time SELECT refinements | Pivot-parametric deterministic SELECT, the local five-element median certificate, executable grouping, and the full-input split-count core are proved against the rank-certificate interface; randomized expected-time analysis, final `7n/10` partition-size packaging, and runtime proof remain. |
