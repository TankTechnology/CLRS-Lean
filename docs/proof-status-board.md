...
| Chapter 7, Section 7.1 | Stable functional partition classification, scan-state partition-loop correctness, returned pivot-index partition postconditions, permutation preservation, and functional quicksort sortedness/permutation preservation are proved. | Swap-trace mutable-array `PARTITION`, deterministic recurrence analysis, randomized quicksort, and expected-time analysis. |
...
| Chapter 9, Sections 9.2-9.3 | Selection-by-rank correctness is proved for the specification selector, a pivot-style quickselect model, and a pivot-parametric deterministic SELECT model, using a count-based order-statistic certificate; the local five-element median certificate, executable grouping, full-input split-count core, and `7n/10 + O(1)` partition-size wrapper are also proved. | Randomized SELECT expected time and runtime recurrence analysis. |
...
| Chapter 7, Sections 7.2-7.4 | Swap-trace mutable-array partition refinement, deterministic performance recurrence, randomized quicksort, and expected-time theorem. |
| Chapter 9 linear-time SELECT refinements | Pivot-parametric deterministic SELECT, the local five-element median certificate, executable grouping, the full-input split-count core, and the `7n/10 + O(1)` partition-size wrapper are proved against the rank-certificate interface; randomized expected-time analysis and runtime proof remain. |
