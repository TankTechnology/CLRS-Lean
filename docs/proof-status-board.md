...
| Chapter 14, Section 14.1 | Order-statistic tree size augmentation, size-field recomputation, key preservation, size/rank-preserving local rotations, cached-root-size preservation, augmented rank selection, and recompute-then-rotate selector preservation are proved for a functional tree.  Section 14.3 also proves interval trees, the general augmentation theorem (CLRS Theorem 14.1, `augmentation_theorem`), a size-augmentation instance unifying 14.1 with the generic framework, and a red-black bridge (`rb_augmentation_bridge`) showing Chapter 13's rotations and recoloring maintain any rotation-invariant augmentation. | Track a stored augmentation field through the concrete executable `RBTree.insert` (value-level maintainability is proved). |
...
| Chapter 14 remaining augmentation targets | Interval trees, the general augmentation theorem (Theorem 14.1), and the red-black rotation bridge are proved; the remaining refinement is tracking a stored augmentation field through the concrete executable `RBTree.insert`. |
