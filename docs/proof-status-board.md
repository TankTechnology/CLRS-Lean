...
| Chapter 15, Sections 15.1, 15.2, 15.4, and 15.5 | Rod-cutting Bellman recurrence facts, finite bottom-up table-prefix correctness, and an executable recurrence-valued rod-cutting function are proved; matrix-chain parenthesization optimality plus split-table reconstruction certificates and reconstructed-plan cost equality, LCS certificate optimality plus table-recurrence reconstruction certificates/wrappers including matching/nonmatching recurrence consequences, and optimal-BST plan optimality plus an executable recurrence-valued OBST function are also proved. | Mutable-array/memoized rod cutting and executable matrix-chain, LCS, and OBST reconstruction algorithms. |
...
| Chapter 22, Sections 22.1-22.5 | A finite directed-graph model with adjacency, walks, paths, cycles, reachability, connected components, and undirected-graph symmetry is established; a fuelled BFS and a soundness proof that every visited vertex is reachable are in place; a functional DFS with discovery/finish times and parent pointers is present, together with the basic color invariants and the global soundness lemma `dfs_all_black`; Kahn's algorithm for topological sorting is implemented and proved correct for DAGs; Kosaraju's SCC algorithm is implemented and the returned components are proved to be a nonempty partition of the vertex set, with the SCC strong-connectivity/maximality core isolated in `kosarajuComponent_scc_core` and admitted. | DFS parenthesis theorem, white-path theorem, and edge classification; BFS completeness and unweighted shortest-path distances; Kosaraju SCC strong-connectivity/maximality core (`kosarajuComponent_scc_core`). |
...
| Chapter 15 remaining dynamic-programming targets | Mutable-array/memoized rod cutting and executable matrix-chain, LCS, and OBST table/reconstruction algorithms. |
...
| --- | --- | --- |
| 0 | Chapter 22 | Complete DFS (parenthesis theorem, white-path theorem, edge classification) and the Kosaraju SCC strong-connectivity/maximality core on the Section 22.1 graph model; BFS completeness and shortest-path distances remain secondary. |
...
| 3 | Chapter 15, Sections 15.2/15.4 plus 15.1 refinements | Advance DP table and reconstruction theorems, prioritizing matrix-chain executable reconstruction and LCS table/reconstruction after the new 15.1 table-certificate layer. |
