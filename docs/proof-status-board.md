...
| --- | --- | --- |
| 0 | Chapter 22 | Complete the remaining textbook-facing DFS layer: edge classification and DFS-based topological sort.  The white-path theorem, timestamp parenthesis theorem, parent-forest ancestor/interval characterization, and full Kosaraju SCC correctness are proved; BFS shortest-path distance lemmas remain secondary. |
