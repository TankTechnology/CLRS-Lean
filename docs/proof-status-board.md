...
| Chapter 22, Sections 22.1-22.5 | A finite directed-graph model with adjacency, walks, paths, cycles, reachability, connected components, and undirected-graph symmetry is established; a fuelled BFS is in place and both soundness and completeness are proved; a functional DFS with discovery/finish times and parent pointers is present, together with the basic color invariants and the global soundness lemma `dfs_all_black`; Kahn's algorithm for topological sorting is implemented and proved correct for DAGs; Kosaraju's SCC algorithm is implemented and the returned components are proved to be a nonempty partition of the vertex set, with the SCC strong-connectivity/maximality core isolated in `kosarajuComponent_scc_core` and reduced to the admitted DFS finish-time lemma `scc_finish_order`. | DFS parenthesis theorem, white-path theorem, and edge classification; BFS shortest-path distance lemmas; Kosaraju SCC strong-connectivity/maximality core (`kosarajuComponent_scc_core` / `scc_finish_order`). |
...
| --- | --- | --- |
| 0 | Chapter 22 | Complete DFS (parenthesis theorem, white-path theorem, edge classification) and the Kosaraju SCC strong-connectivity/maximality core on the Section 22.1 graph model; BFS shortest-path distance lemmas remain secondary. |
