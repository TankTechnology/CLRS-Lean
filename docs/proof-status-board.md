...
| Chapter 22, Sections 22.1-22.2 | A finite directed-graph model with adjacency, walks, paths, cycles, reachability, connected components, and undirected-graph symmetry is established; a fuelled BFS and a soundness proof that every visited vertex is reachable are in place. | BFS completeness and unweighted shortest-path distances; DFS correctness; topological sort; strongly connected components. |
...
| --- | --- | --- |
| 0 | Chapter 22 | Complete BFS (reachability completeness and shortest-path distances), then DFS, topological sort, and strongly connected components on the Section 22.1 graph model. |
