...
| Chapter 7, Sections 7.1-7.3 | All functional partition/quicksort theorems, deterministic quadratic bounds, expected-comparison recurrence with harmonic bounds, AND the random-permutation probability model (`isFirst_prob`, `compared_prob = 2/(j-i+1)`) are proved. | Index-level mutable-array `PARTITION`, asymptotic Θ(n log n) bridge from probability model. |
...
| --- | --- | --- | --- |
| Extreme | Randomized expected-time analysis (Chapters 5, 7, 8, 11) | ✅ Unified finite expectation toolkit created (`CLRSLean/Probability/FiniteExpectation.lean`). Ch5 migrated, Ch7 random-permutation symmetry lemma proved, Ch8.4/Ch11.2 toolkit imported. | Extend to Ch9 randomized SELECT, Ch11.2 full probability model. |
