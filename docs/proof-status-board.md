...
| Chapter 22, Sections 22.1-22.5 | A finite directed-graph model and reachability-complete BFS are established; the DFS development proves color/timestamp invariants, the white-path characterization, the timestamp parenthesis theorem, the parent-forest ancestor/interval characterization, unique tree/back/forward/cross edge classification, discovery-state bridges, and SCC finish-time ordering; Kahn topological sort is proved correct for DAGs; Kosaraju's returned blocks are proved to be strongly connected, maximal, pairwise disjoint, covering, and therefore an SCC partition. | BFS shortest-path distance and predecessor-tree lemmas; a DFS-based topological sort matching the CLRS presentation; explicit cost refinements. |
...
| --- | --- | --- |
| 0 | Chapter 22 | Complete the remaining textbook-facing DFS-based topological sort.  The white-path theorem, timestamp parenthesis theorem, parent-forest ancestor/interval characterization, unique edge classification, and full Kosaraju SCC correctness are proved; BFS shortest-path distance lemmas remain secondary. |
