...
| `literate.toml` rendering configuration | `render` | Restore compiled inputs and renderer, render four shards, assemble and validate |
| Lean source, dependencies, renderer patches, unknown inputs, or unavailable compatible artifacts | `full` | Compile Lean, render four shards, assemble and validate |

The asset path preserves proof-page HTML byte-for-byte. Deleted owned assets are
removed. All refresh paths retain the reader checks and Chromium smoke tests.
...
Workflow edits also require a full build. The September 14 routing and
render-only migrations have narrow exceptions pinned to exact old/new workflow
content hashes in `scripts/site_deploy.py`; any other version invalidates them.
...
`literate-inputs` retains the compiled JSON and renderer for 90 days. A render
refresh replans these inputs against the current configuration and uses four
separate matrix runners. Lean setup, dependency downloads and compilation are
skipped. `compiled-revision.txt`, `content-revision.txt`, and `revision.txt`
identify the original compilation, current raw rendering, and published site
respectively. The local `site_deploy.py refresh` fallback renders sequentially
to bound memory use; CI runs the four shards in parallel.
...
Before composition, `reader_implementation.py` inventories every canonical
section's source directory and its owning row in the fourth-edition mapping.
It includes all split implementation files, including files not listed in the
navigation, even when the section already has declarations. Shared mapped
sources are expanded in the same way. Empty import facades resolve to their
first declaration-bearing implementation pages. Prose result links resolve to
visible declarations; ordinary dependencies inside proofs retain source links.
The existing rendered definitions and complete proofs are copied without
generating or changing any Lean code.
...
`audit_reader_book.py` checks every chapter and canonical section, cross-page
fragment links, and preservation of standalone prose/code in its chapter body.
With `--compiled .lake/build/literate`, it also compares every compiled
definition-bearing command against visible code text, checks every owned
companion, and independently verifies the curated declaration selections.
Hover diagnostics and coverage JSON do not count as proof text. Missing required
compiled inputs fail the audit. Assembly produces `reader-book-audit.json`;
render/full deployment adds the compiled-command evidence to that report.

Do not hide the broad `set_option` command pattern in `literate.toml`: the
renderer matches scoped `set_option ... in` commands too, hiding the entire
wrapped theorem. The compiled-body audit detects this loss.
...
The whole-book browser check visits all 35 chapters and 136 canonical sections
at desktop and mobile widths with JavaScript disabled, and records every page's
result along with one mobile screenshot per chapter.
